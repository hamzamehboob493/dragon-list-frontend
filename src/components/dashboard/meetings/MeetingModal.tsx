...
import { Meeting, MeetingModalProps, Team, User } from '@/lib/types/dashboard/types';

const MeetingModal: React.FC<MeetingModalProps> = ({ isOpen, onClose, onSubmit, meeting = null, loading = false }) => {
  const [teams, setTeams] = useState<Team[]>([]);
  const [users, setUsers] = useState<User[]>([]);
  const [teamsLoading, setTeamsLoading] = useState(false);
  const [usersLoading, setUsersLoading] = useState(false);

  const { register, handleSubmit, formState: { errors }, reset, setValue, watch } = useForm<Meeting>({
  ...
  useEffect(() => {
    if (meeting) {
      ...
      setValue('startTime', meeting.startTime ? new Date(meeting.startTime).toISOString().slice(0, 16) : '');
      setValue('endTime', meeting.endTime ? new Date(meeting.endTime).toISOString().slice(0, 16) : '');
      ...
      setValue('originalStartTime', meeting.originalStartTime ? new Date(meeting.originalStartTime).toISOString().slice(0, 16) : '');
      setValue('recurrenceEndDate', meeting.recurrenceEndDate ? new Date(meeting.recurrenceEndDate).toISOString().slice(0, 16) : '');
  ...
  const handleFormSubmit: SubmitHandler<Meeting> = (data) => {
    onSubmit({
      ...
      originalStartTime: data.originalStartTime ? new Date(data.originalStartTime).toISOString() : '',
      recurrenceEndDate: data.recurrenceEndDate ? new Date(data.recurrenceEndDate).toISOString() : '',
    });
    onClose();
    if (!meeting) reset();
  };
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-3xl max-h-screen overflow-y-auto">
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Google Meet ID</label>
              <input
                type="text"
                ...
                disabled={loading || teamsLoading || usersLoading}
              />
              {errors.googleMeetId && <p className="text-red-500 text-sm mt-1">{errors.googleMeetId.message}</p>}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Google Doc ID</label>
              <input
                type="text"
                ...
                disabled={loading || teamsLoading || usersLoading}
              />
              {errors.googleDocId && <p className="text-red-500 text-sm mt-1">{errors.googleDocId.message}</p>}
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Google Drive Folder ID</label>
              <input
                type="text"
                ...
                disabled={loading || teamsLoading || usersLoading}
              />
              {errors.googleDriveFolderId && <p className="text-red-500 text-sm mt-1">{errors.googleDriveFolderId.message}</p>}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              {errors.teamId && <p className="text-red-500 text-sm mt-1">{errors.teamId.message}</p>}
            ...
            <div>
              ...
              {errors.organizerId && <p className="text-red-500 text-sm mt-1">{errors.organizerId.message}</p>}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Original Start Time</label>
              <input
                type="datetime-local"
                ...
                disabled={loading || teamsLoading || usersLoading}
              />
              {errors.originalStartTime && <p className="text-red-500 text-sm mt-1">{errors.originalStartTime.message}</p>}
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Recurrence End Date</label>
              <input
                type="datetime-local"
                ...
                disabled={loading || teamsLoading || usersLoading}
              />
              {errors.recurrenceEndDate && <p className="text-red-500 text-sm mt-1">{errors.recurrenceEndDate.message}</p>}
            </div>
          </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  {errors.recurrencePattern && <p className="text-red-500 text-sm mt-1">{errors.recurrencePattern.message}</p>}
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Max Occurrences</label>
                  <input
                    type="number"
                    ...
                    disabled={loading || teamsLoading || usersLoading}
                  />
                  {errors.maxOccurrences && <p className="text-red-500 text-sm mt-1">{errors.maxOccurrences.message}</p>}
                </div>
              </div>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Participant Count</label>
              <input
                type="number"
                ...
                disabled={loading || teamsLoading || usersLoading}
              />
              {errors.participantCount && <p className="text-red-500 text-sm mt-1">{errors.participantCount.message}</p>}
            </div>
            ...
            <div>
              ...
              <input
                ...
                disabled={loading || teamsLoading || usersLoading}
              />
              {errors.recordingUrl && <p className="text-red-500 text-sm mt-1">{errors.recordingUrl.message}</p>}
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Is Exception</label>
              <input
                type="checkbox"
                ...
                disabled={loading || teamsLoading || usersLoading}
              />
              {errors.isException && <p className="text-red-500 text-sm mt-1">{errors.isException.message}</p>}
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Series ID</label>
              <input
                type="text"
                {...register('seriesId')}
                className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500"
                disabled={loading || teamsLoading || usersLoading}
              />
              {errors.seriesId && <p className="text-red-500 text-sm mt-1">{errors.seriesId.message}</p>}
            </div>
          </div>
