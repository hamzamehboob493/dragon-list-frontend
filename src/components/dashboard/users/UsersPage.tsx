    ...
    try {
      ...
      if (editingUser?.id) {
        response = await updateAction(
          `${routes.api.users.index}/${editingUser.id}`,
          data,
        );
        if (response?.status === 200) {
          await getUsersData();
                ...
                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                  {filteredUsers.map((user) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span
                          className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getRoleBadge(user.role?.name as string)}`}
                        >
                          {user.role?.name}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span
                          className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getStatusBadge(user.status?.name as string)}`}
                        >
                          {user.status?.name}
                        </span>
                      </td>
